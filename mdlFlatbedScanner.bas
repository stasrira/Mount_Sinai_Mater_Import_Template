...
'    Call get_hostname
    ...
    If ValidateSocketOperation(SendCommand("read rackid"), Send_sc, "Current SendCommand: read rackid." & vbCrLf & msgVerifyTracxerNote) < 0 Then Exit Sub
'    x = SendCommand("scan box")
    Sleep (500) 'wait until scan is completed
    If ValidateSocketOperation(RecvAscii(recvBuf, 1024), Recv_sc, "Current SendCommand: read rackid.") < 0 Then Exit Sub
...
'    x = RecvAscii(recvBuf, 1024) '1024
    ...
    If ValidateSocketOperation(SendCommand("state"), Send_sc, "Current SendCommand: state.") < 0 Then Exit Sub
'    x = SendCommand("state")
    If ValidateSocketOperation(RecvAscii(recvBuf, 1024), Recv_sc, "Current SendCommand: state.") < 0 Then Exit Sub
'    x = RecvAscii(recvBuf, 1024) '1024
...
Function Read_Simple_Scanner_Output(input_data As String) As String
    Dim input_arr() As String
    Dim output_val As String, command_line As String
    Dim i As Integer
    ...
    'The proper command line has to be treated as following:
    'The output is space delimited. First element is the status and the last element is the returned valule.
    'Examples:
    ...
    For i = UBound(input_arr) To LBound(input_arr) Step -1
        If Len(Trim(input_arr(i))) > 0 Then Exit For
    Next
    
    'save found command line in the variable
    command_line = input_arr(i)
    
    If Left(command_line, 2) = Scanner_OKStatus Then
        
        input_arr = Split(command_line, " ")
        'check if the last element of input_arr is not blank and pass it back to the caller of the function
        If Len(Trim(input_arr(UBound(input_arr)))) > 0 Then
            output_val = Trim(input_arr(UBound(input_arr)))
        End If
    End If
...
Sub Read_Scan_Results(input_data As String, Optional rackId_val As String = "")
    ...
    If Left(input_data, 2) = Scanner_OKStatus Then
        
        input_arr = Split(input_data, Scanner_GetScanResults)
        
        If UBound(input_arr) = 1 Then
    
            'clean returned data string - remove any preceding info, update header of the last column, trim spaces
            'last element of the array will keep the actual scan results from the scanner, that will be saved back to the input_data
            input_data = input_arr(UBound(input_arr))
            'replace header of the last column returned from the scanner (Line End) with the expected value of this column ("end text")
            input_data = Replace(input_data, Scanner_LineEnd_Header, Scanner_LineEnd_Value)
            'trim extra spaces
            input_data = Trim(input_data)
            
            'split the cleanned set of results by Line End Value
            ...
            For i = LBound(input_arr) To UBound(input_arr)
                ...
                'incorporate rackID to each line as the last provided value; do not add rackid to the header line (the first line)
                If Len(Trim(rackId_val)) > 0 And i > 0 Then
                    cur_range.Value = cur_range.Value & rackId_val
                End If
